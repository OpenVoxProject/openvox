...
want to apply. OpenVox elements are tagged with all of the class or
...
## OPTIONS
Note that any OpenVox setting that\'s valid in the configuration file is
...
- \--certname: Set the certname (unique ID) of the client. The server
  ...
  implement unusual node identification schemes. (This is an OpenVox
  setting, and can go in puppet.conf.)

- \--daemonize: Send the process into the background. This is the
  default. (This is an OpenVox setting, and can go in puppet.conf. Note
...
- \--no-daemonize: Do not send the process into the background. (This is
  an OpenVox setting, and can go in puppet.conf. Note the special
...
- \--evaltrace: Logs each resource as it is being evaluated. This allows
  you to interactively see exactly what is being done. (This is an
  OpenVox setting, and can go in puppet.conf. Note the special \'no-\'
...
- \--masterport: The port on which to contact the OpenVox Server. (This
  is an OpenVox setting, and can go in puppet.conf. Deprecated in favor
  of the \'serverport\' setting.)

- \--noop: Use \'noop\' mode where the daemon runs in a no-op or dry-run
  mode. This is useful for seeing what changes OpenVox would make
  without actually executing the changes. (This is an OpenVox setting,
...
- \--onetime: Run the configuration once. Runs a single (normally
  daemonized) OpenVox run. Useful for interactively running puppet agent
  when used in conjunction with the \--no-daemonize option. (This is an
  OpenVox setting, and can go in puppet.conf. Note the special \'no-\'
  prefix for boolean settings on the command line.)

- \--serverport: The port on which to contact the OpenVox Server. (This
  is an OpenVox setting, and can go in puppet.conf.)

- \--sourceaddress: Set the source IP address for transactions. This
  defaults to automatically selected. (This is an OpenVox setting, and
...
- \--trace Prints stack traces on some errors. (This is an OpenVox
  setting, and can go in puppet.conf. Note the special \'no-\' prefix
  for boolean settings on the command line.)

- \--verbose: Turn on verbose reporting.

- \--version: Print the OpenVox agent version number and exit.
...
- \--waitforcert: This option only matters for daemons that do not yet
  ...
  the initial setup of an OpenVox client. You can turn off waiting for
  certificates by specifying a time of 0. (This is an OpenVox setting,
...
- \--write_catalog_summary After compiling the catalog saves the
  ...
  named classes.txt and resources.txt (This is an OpenVox setting, and
  can go in puppet.conf.)
...
:   Immediately retrieve and apply configurations from the OpenVox
    server.
