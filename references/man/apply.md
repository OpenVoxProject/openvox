...
the full list of acceptable parameters. You can generate a commented
list of all configuration options by running puppet with
\'\--genconfig\'.
...
- \--noop: Use \'noop\' mode where OpenVox runs in a no-op or dry-run
  mode. This is useful for seeing what changes OpenVox will make without
...
- \--catalog: Apply a JSON catalog (such as one generated with \'puppet
  catalog compile\'). You can either specify a JSON file or pipe in JSON
  from standard input.
